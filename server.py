...
def markt_auswerten(m):
    try:
        ...
        try:
            ...
            tage     = max(0, (dt_de - jetzt_de).days)
            # Formatiert: "25.04.2026 um 21:00"
...
def pipeline():
    while True:
        ...
        # Top 10: nur Wetten die in 1–14 Tagen enden, beste Gewinnchance zuerst
        # Keine vergangenen Märkte (tage > 0), keine die zu weit weg sind (tage <= 14)
        top10 = sorted(
            [e for e in empfehlungen if 1 <= e["tage"] <= 14],
            key=lambda x: x["gewinn_chance"],
            reverse=True
        )[:10]

        # Falls weniger als 10 in 14 Tagen: auf 30 Tage erweitern
        if len(top10) < 10:
            weitere = sorted(
                [e for e in empfehlungen if 1 <= e["tage"] <= 30 and e not in top10],
                key=lambda x: x["gewinn_chance"],
                reverse=True
            )
            top10 = (top10 + weitere)[:10]

        # "Bald endend"-Tab bleibt gleich (innerhalb 14 Tage, nach Tagen sortiert)
        top10_bald = sorted(
            [e for e in empfehlungen if 1 <= e["tage"] <= 14],
            key=lambda x: x["tage"]
        )[:10]
        if len(top10_bald) < 10:
            weitere = sorted(
                [e for e in empfehlungen if 1 <= e["tage"] <= 30 and e not in top10_bald],
                key=lambda x: x["tage"]
            )
            top10_bald = (top10_bald + weitere)[:10]
