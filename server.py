...
def markt_auswerten(m):
    try:
        ...
        return {
            ...
            "link":      polymarket_link(m),   # ← direkter Link
        }
    except Exception:
        return None
    ...
    while True:
        ...
        for m in roh:
            ...
            eintrag = {
                "id":          markt["id"],
                "frage":       markt["frage"],
                "link":        markt["link"],        # ← Polymarket-Link
                ...
                "einsatz":     einsatz,
                "erw_gewinn":  gewinn,
                "gewinn_chance": chance,             # ← Gewinnchance in %
        ...
        # Top 10 nach höchster Gewinnchance
        empfehlungen.sort(key=lambda x: abs(x["edge"]), reverse=True)
        top10 = sorted(empfehlungen, key=lambda x: x["gewinn_chance"], reverse=True)[:10]

        # Top 10 die bald enden (niedrigste Tage zuerst, min. 1 Tag, max. 30 Tage)
        top10_bald = sorted(
            [e for e in empfehlungen if 1 <= e["tage"] <= 30],
            key=lambda x: x["tage"]
        )[:10]
        # Falls weniger als 10 kurze: auffüllen mit bis 60 Tage
        if len(top10_bald) < 10:
            weitere = sorted(
                [e for e in empfehlungen if e["tage"] <= 60 and e not in top10_bald],
                key=lambda x: x["tage"]
            )
