...
def pipeline():
    while True:
        ...
        for m in roh:
            ...
            # Richtung und Gewinnchance für ALLE Märkte berechnen (nicht nur mit Edge)
            if richt is None:
                richt = "JA" if modell_p >= markt["ja_preis"] else "NEIN"
            chance = gewinn_chance(modell_p, richt, markt["ja_preis"])
        ...
        # Top 10 bald endend: aus ALLEN gültigen Märkten (nicht nur mit Edge)
        # → höchste Gewinnchance, sortiert nach bald endend
        def top10_aus(pool, max_tage):
            return sorted(
                [e for e in pool if e["tage"] <= max_tage],
                key=lambda x: x["gewinn_chance"], reverse=True
            )[:10]

        top10_bald = top10_aus(alle, 7)
        if len(top10_bald) < 10:
            ids = {e["id"] for e in top10_bald}
            top10_bald = (top10_bald + top10_aus(
                [e for e in alle if e["id"] not in ids], 30))[:10]
        if len(top10_bald) < 10:
            ids = {e["id"] for e in top10_bald}
            top10_bald = (top10_bald + top10_aus(
                [e for e in alle if e["id"] not in ids], 90))[:10]
        if len(top10_bald) < 10:
            ids = {e["id"] for e in top10_bald}
            rest = sorted(
                [e for e in alle if e["id"] not in ids],
                key=lambda x: x["gewinn_chance"], reverse=True
            )
            top10_bald = (top10_bald + rest)[:10]
